...
def sauvegarder_base():
    ...
    try:
        ...
        with open(backup_file, 'w', encoding='utf-8') as f:
            ...
            f.write("ALTER TABLE produits AUTO_INCREMENT = 1;\n\n")
            
            for p in produits:
                f.write(f"INSERT INTO produits (nom, code_barres, prix, stock) VALUES ('{p[1]}', '{p[2]}', {p[3]}, {p[4]});\n")
...
def debug():
    ...
    try:
        ...
        html = "<h1>🔍 DEBUG - Données de la base</h1>"
        html += f"<p><strong>Nombre de produits:</strong> {len(produits)}</p>"
        html += "<table border='1' style='border-collapse: collapse; width: 100%;'>"
        html += "<tr><th>ID</th><th>Nom</th><th>Code-barres</th><th>Prix</th><th>Stock</th></tr>"
        
        for p in produits:
            html += f"<tr><td>{p[0]}</td><td>{p[1]}</td><td>{p[2]}</td><td>{p[3]:.2f}€</td><td>{p[4]}</td></tr>"
...
def ajouter():
    """Ajouter un produit SIMPLE avec validation complète"""
    if request.method == 'POST':
        try:
            ...
            print(f"✅ PRODUIT AJOUTÉ: ID={produit_id}, Nom='{nom}', Code='{code_barres}', Prix={prix}€, Stock={stock}")
...
def export_rupture():
    ...
    try:
        ...
        # En-têtes avec informations détaillées
        writer.writerow([
            'ID', 'Nom du Produit', 'Code-barres', 'Prix Unitaire (€)', 
            'Stock Actuel', 'Statut', 'Action Requise', 'Date Export'
        ])
