...
# Base de données SQLite avec chemin absolu
import tempfile
DATABASE_DIR = os.environ.get('DATABASE_DIR', '/tmp')
DATABASE = os.path.join(DATABASE_DIR, 'inventaire.db')
...
def export_stock_faible():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM produits WHERE stock <= 5 ORDER BY stock ASC')
        produits = cursor.fetchall()
        conn.close()
        ...
        writer.writerow(['Nom', 'Code-barres', 'Prix', 'Stock', 'Statut'])
        
        for produit in produits:
            statut = 'RUPTURE' if produit['stock'] == 0 else 'CRITIQUE' if produit['stock'] <= 2 else 'FAIBLE'
            writer.writerow([produit['nom'], produit['code_barres'], produit['prix'], produit['stock'], statut])
        ...
        return Response(
            output.getvalue(),
            mimetype='text/csv',
            headers={'Content-Disposition': f'attachment; filename=stock_faible_{datetime.now().strftime("%Y%m%d")}.csv'}
        )
...
def export_rupture():
    """Export CSV des produits en rupture"""
    try:
        conn = get_connection()
        if not conn:
            return "Erreur de connexion", 500
        ...
        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(['Nom', 'Code-barres', 'Prix', 'Stock'])
        
        for produit in produits:
            writer.writerow([produit['nom'], produit['code_barres'], produit['prix'], produit['stock']])
        ...
        return Response(
            output.getvalue(),
            mimetype='text/csv',
            headers={'Content-Disposition': f'attachment; filename=ruptures_{datetime.now().strftime("%Y%m%d")}.csv'}
        )
...
def sauvegarde():
    ...
    try:
        ...
        return jsonify({
            'success': True, 
            'message': f'💾 Base sauvegardée ! ({count} produits)'
        })
