...
Application de gestion d'inventaire avec scanner - SANS ERREUR 500
"""
...
def index():
    ...
    try:
        ...
        if conn:
            ...
            try:
                ...
                # Fallback HTML intégré avec données
                produits_html = ""
                for p in produits:
                    produits_html += f"""
    ...
    # Page HTML simple en cas d'erreur totale
    return """
    <!DOCTYPE html>
    <html lang="fr">
    <head>
...
@app.route('/scanner')
def scanner():
    """Scanner avec caméra ou fallback simple"""
    try:
        return render_template('scanner_parfait.html')
    except:
        # Scanner simple intégré
        return """
        <!DOCTYPE html>
        <html lang="fr">
        <head>
...
def api_scan():
    """API de scan avec base de données"""
    try:
